...
class pages{
    ...
    function out(){
        ...
        $str="<tr><td colspan='8'><div class='pagelist'><a href='javascript:;'>一共有{$this->total}条</a>";
        $str.="<a href='javascript:;'>一共{$this->pageNums}页</a>";
        ...
        for($i=$start;$i<$end;$i++){
            $n=$i+1;
            if($i==$this->current){
                $str.="<a href='{$url}{$i}' class='current'>".$n."</a>";
            }else{
        ...
        $str.="<a href='{$url}{$last}'>尾页</a></div></td></tr>";
        $this->limit="limit ".$this->current*$this->pageNum.",".$this->pageNum;
        return $str;
    }
}
//$obj=new pages(10,2,"pages");
